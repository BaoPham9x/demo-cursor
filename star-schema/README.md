...
- `models/marts/schema.yml` — **semantic layer source for agents**: join paths, Steep module identifiers, dimension types, example values, and reference metric recipes distilled from the internal generator + existing Steep modules.
...
For the **sales demo without BigQuery**, the agent only needs **`models/marts/schema.yml`** (and optionally these `.sql` files for column lists). No separate sample CSVs live in the demo repo.

...
- Map `meta.steep.join_paths` to YAML `joinPaths` on the **module that holds the foreign key**, with `from.column` / `to.table` / `to.column` / `type` (`one-to-many` or `one-to-one` per Steep rules).
